...
// Tamaño compacto (Filas x Columnas) solicitado 12x8
...
function playSound(type) {
  ...
  switch (type) {
    case 'click': t("C4"); break;
    case 'select': t("C5"); break;
    case 'confirm': t("G5"); break;
    case 'start': t("C4"); setTimeout(()=>t("G4"),100); setTimeout(()=>t("C5"),200); break;
    case 'win': t("G5"); setTimeout(()=>t("C6"),100); break;
    case 'success': t("C5"); setTimeout(()=>t("E5"),100); setTimeout(()=>t("G5"),200); break;
    case 'failure': t("A3"); break;
  }
}
...
async function createGame() {
  ...
  const gameData = {
    ...
    gameSettings: {
      roundLimit: 10,
      scoreLimit: 25,
      boardMode: 'full' // default, el host puede cambiar en la sala
    },
    createdAt: new Date()
  };
...
function makeViewport(mode) {
  if (mode === 'compact') {
    const maxRowStart = FULL_ROWS - COMPACT_ROWS;  // 4
    const maxColStart = FULL_COLS - COMPACT_COLS;  // 22
...
function updateUI(gameData) {
  if (gameData.gameState === 'waiting') {
    ...
    if (isHost) {
      ...
      roundLimitInput.disabled = false; scoreLimitInput.disabled = false; boardSizeSelect.disabled = false;
  ...
  gameTitle.textContent = (names.length === 2) ? `${names[0]} vs ${names[1]}` : 'Partida Grupal';
...
/* ============ GRID RENDER ============ */
let lastGridKey = ""; // para no re-generar innecesariamente
function renderBoard(gameData) {
  // limpiar marcadores y frames
  ...
  if (gridKey !== lastGridKey) {
    buildGrid(viewport); // vuelve a pintar
    lastGridKey = gridKey;
  }
  ...
  // resaltar color secreto solo al dador (si ya eligió)
  ...
  if (gameData.guesses) {
    if (gameData.gameState === 'scoring' || gameData.gameState === 'gameOver' || gameData.gameState === 'roundSummary' || isCueGiver) {
      Object.entries(gameData.guesses).forEach(([pid, guesses]) => {
        const p = gameData.players[pid];
        guesses.forEach(guess => drawMarker(guess, p));
      });
    } else {
...
function buildGrid(viewport) {
  ...
  for (let r = 0; r < viewport.rows; r++) {
    ...
    for (let c = 0; c < viewport.cols; c++) {
      ...
      cell.dataset.x = x; // coordenada real en el 30x16
      cell.dataset.y = y;
...
function renderGameInfo(gameData) {
  ...
  if (gameData.gameState.includes('guessing')) {
    ...
    if (pending.length > 0) html += `<p class="text-yellow-400 font-semibold">Turno de adivinar de:<br>${pending.join(', ')}</p>`;
    else html += `<p class="text-gray-400 font-semibold">Todos han adivinado.</p>`;
  } else if (gameData.gameState === 'choose_secret') {
  ...
  if (currentUserId === cueGiverId && gameData.gameState !== 'scoring') {
    ...
    // Mostrar 4 opciones cuando toca elegir
    if (gameData.gameState === 'choose_secret' && Array.isArray(gameData.candidateCards)) {
      ...
      gameData.candidateCards.forEach((cand, idx) => {
        ...
        b.className = 'w-8 h-8 rounded-md border-2 border-gray-500';
...
function createClueInputHTML(n) {
  ...
  wrapper.querySelector('#clue-input').addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { e.preventDefault(); submitClue(n); }
  });
...
async function handleGridClick(e) {
  ...
  renderBoard(gameData); renderControls(gameData);
}
...
document.getElementById('next-round-btn').addEventListener('click', async () => {
  ...
  // Nuevo viewport si el modo es compacto
...
function showGameOver(gameData) {
  const winner = gameData.playerOrder.reduce((a, b) =>
    gameData.players[a].score > gameData.players[b].score ? a : b);
